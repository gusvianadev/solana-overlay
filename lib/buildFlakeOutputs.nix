...
{
# Override packages
buildToolOverrides ? { }, # Setup overrides from packages
setupBuildToolOverrides ? { pkgs }: { }, # Set up build tools
...
flake-utils.lib.eachSystem supportedSystems (system:
  let
    ...
    tool-anchor = if (builtins.hasAttr "anchor" buildTools) then
      buildTools.anchor
    else
      pkgs.anchor;
    tool-solana = if (builtins.hasAttr "solana" buildTools) then
      buildTools.solana
    else
      pkgs.solana-basic;
